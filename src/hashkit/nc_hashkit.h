...
uint32_t hash_redis_slot(const char *key, size_t key_length);
...
rstatus_t redis_slot_update(struct server_pool *pool);
uint32_t redis_slot_dispatch(struct continuum *continuum, uint32_t ncontinuum, uint32_t hash);
